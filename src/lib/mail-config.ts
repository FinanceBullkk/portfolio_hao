...
export async function loadMailConfig(): Promise<MailConfig> {
  ...
  const m = (snap.exists() ? (snap.data().mail as Partial<MailConfig> | undefined) : undefined) ?? {};
  ...
  for (const [k, v] of Object.entries(rawTemplates)) {
    templates[k] = {
      title: str((v as MailTemplate)?.title, ''),
      subject: str((v as MailTemplate)?.subject, ''),
      bodyHtml: str((v as MailTemplate)?.bodyHtml, ''),
    };
  }
  return {
