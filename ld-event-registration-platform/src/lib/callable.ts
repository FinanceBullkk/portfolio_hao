...
export async function callable<Req, Res>(name: string, args: Req): Promise<Res> {
  ...
  try {
    ...
    // Add a 250ms timeout to prevent hanging when Cloud Functions are not deployed or network is slow
    const timeout = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error(`Callable "${name}" request timed out`)), 250)
    );
